...
@register(outgoing=True, pattern="^.(song|lagu|spoty)(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return
    link = event.pattern_match.group(2)
    chat = "@SpotifyMusicDownloaderBot"
    await event.edit("Searching music...")
    async with bot.conversation(chat) as conv:
          ...
          try:
              ...
              await event.delete()
              await event.reply("Unblock @SpotifyMusicDownloaderBot and try again.")
              return
          await event.delete()
