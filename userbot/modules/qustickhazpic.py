...
async def mim(event):
    ...
    async with bot.conversation("@MemeAutobot") as bot_conv:
          ...
          if response.text.startswith("Forward"):
              await event.edit("```Failed due to privacy```")
...
async def quotes(qotlti):
    ...
    async with bot.conversation(chat) as conv:
          ...
          if response.text.startswith("Hi!"):
             await qotlti.edit("```Failed due to privacy```")
          else: 
             await qotlti.delete()   
...
async def hazmat(hazzz):
    ...
    if not hazzz.reply_to_msg_id:
       await hazzz.edit("`WoWoWo Capt!, we are not going suit a pocong!...`")
       return
...
CMD_HELP.update({
    "quote":
    ">`.pch`"
    "\nUsage: Enhance ur text to sticker(antiddos.systems).\n"
    ">`.q` \
      \nUsage: Enhance ur text to sticker(@QuotLyBot)."
})
